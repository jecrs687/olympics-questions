...
signed main()
{
    ...
    while (n != 0 || m != 0)
    {
        ...
        // g.floydWarshall();
        //  g.printMatrix();
        vi minInitial = g.dijkstra(from);
        vi minReverse = g.dijkstra(to, true);

        int min = minInitial[to];
        vi allValues = vi();
        allValues.resize(n);
        for (int i = 0; i < n; i++)
        {
            ...
            [](auto &&x)
            {
                for_each(
                    ...
                    [](auto &&i)
                    {
                        if (g.adj[x][i] == INF)
                            continue;
                        if (minInitial[x] + g.adj[x][i] + minReverse[i] != min)
                            g2.addEdge(x, i, g.adj[x][i]);
                    });
            });

        int minValue = g2.dijkstra(from)[to];
        if (minValue == INF)
            cout << -1 << endl;
        else
            cout << minValue << endl;
