...
signed main()
{
    ...
    cin >> n;
    cin >> m;
    int maxOfBoth = max(n, m);
    int minOfBoth = max(n, m);

    int sizeOfMax = NumberOfDigits(maxOfBoth);
    int sizeOfMin = NumberOfDigits(minOfBoth);
    ...
    while (n > 0)
    {
        ...
        if (maxOfBoth == m)
        {
            while (sizeOfMax != sizeOfMin)
            {
                int value = (int)m / pow(10, (sizeOfMax - 1));
                int sizeToR = int(pow(10, (sizeOfMax - 1)));
                m = m % sizeToR;
                if (value <= 2)
                    cout << 0;
                else
        ...
        if (maxOfBoth == n)
        {
            while (sizeOfMax != sizeOfMin)
            {
                int value = (int)n / pow(10, (sizeOfMax - 1));
                int sizeToR = int(pow(10, (sizeOfMax - 1)));
                n = n % sizeToR;
                if (value <= 2)
                    cout << 0;
        ...
        int value2 = (int)m / pow(10, (sizeOfMin - 1));
        int value = (int)n / pow(10, (sizeOfMin - 1));
        int sizeToR = int(pow(10, (sizeOfMin - 1)));

        n = n % sizeToR;
        m = m % sizeToR;
        sizeOfMin--;
        if ((value2 <= 2 && value <= 2) || (value2 >= 7 && value >= 7))
            cout << 0;
        else
            cout << 9;
    }
